  ...
  <section class="p-4 sm:p-6 lg:p-8">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button @click="exportCSV" :disabled="!filtered.length" class="px-3 py-2 rounded-md bg-sky-600 hover:bg-sky-700 disabled:opacity-50 text-white text-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="card">
        ...
        <div v-if="range === '24h'" class="grid grid-cols-6 gap-1">
          <div v-for="(v, h) in byHour" :key="h" class="h-6 rounded" :style="{ backgroundColor: heatColor(v) }" :title="`${h}:00 — ${v}`"></div>
        </div>
        <div v-else class="text-sm text-gray-400">Selecione "Últimas 24h" para ver o mapa de calor por hora.</div>
      </div>
    </div>
    ...
    <div class="card mt-6">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
          ...
          <tbody>
            <tr v-if="loading"><td colspan="6" class="px-4 py-6 text-center text-gray-400">Carregando...</td></tr>
            <tr v-else-if="error"><td colspan="6" class="px-4 py-6 text-center text-red-400">{{ error }}</td></tr>
            <tr v-else-if="!filtered.length"><td colspan="6" class="px-4 py-6 text-center text-gray-400">Nenhum registro.</td></tr>
            <tr v-for="(inc, i) in filtered.slice(0, 20)" :key="i" class="border-t border-white/10 hover:bg-white/5">
              <td class="px-4 py-3 text-gray-200">{{ inc.date }}</td>
              <td class="px-4 py-3 text-gray-200">{{ inc.time }}</td>
              <td class="px-4 py-3">
                <span :class="severityPillClass(inc.severity)" class="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-semibold">{{ inc.severity }}</span>
              </td>
