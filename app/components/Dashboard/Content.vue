      ...
      <div class="xl:w-1/4 min-w-[320px] flex flex-col gap-4">
        ...
        <div class="status-cards-row">
          <Card class="status-card compact">
            ...
            <template #content>
              <Tag :severity="getSeverityTag(status.level)" :value="status.level || 'N/A'" class="text-xs px-2 py-1" />
            </template>
          </Card>
          ...
          <Card class="status-card compact">
            ...
            <template #content>
              <p class="text-lg font-semibold text-white">{{ (status.max_confidence * 100).toFixed(0) }}%</p>
            </template>
          </Card>
          ...
          <template #content>
            <ScrollPanel style="width: 100%; height: 180px">
              <div v-if="status.logs.length" class="flex flex-col gap-2 pr-2">
                <p v-for="(log, index) in status.logs.slice().reverse()" :key="index" class="text-sm font-mono text-gray-400 border-b border-gray-700 pb-1">
                  {{ log }}
                </p>
              </div>
...
const videoFeedUrl = ref(`${API_BASE_URL}/video_feed`);
...
const isRecording = computed(() => status.level === 'HIGH' || status.level === 'MILD');
...
const fetchStatus = async () => {
  try {
    ...
    Object.assign(status, { level: 'ERROR', alert: 'Conexão perdida', logs: [], detections: 0, max_confidence: 0 });
  }
};
